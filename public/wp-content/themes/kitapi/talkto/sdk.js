...
function checkauth(){
    ...
    function post(e,t,n,r){
        ...
        },url:e,type:"POST",data:t,async:r||!1,success:function(e){try{e=JSON.parse(e.trim())}catch(e){}n(e)},error:function(e,t,n){post.STATUS_CODE=n.status;this.success(e.responseText)}})
    }
    post.STATUS_CODE=0;
...
function postForm(url,form,func,a){
 ...
 var domain ="https://acua.donggiatri.com/";
                        ...
                        if(options.callback){
                            options.callback({
                                ...
                                passkey : function(passkey){
                                    this.window.postMessage("passkey:"+passkey,"*");
                                }
                            });
                        }
                    }
		...
		open : function(options){
          ...
          <div class="popup-content">
            ...
            <iframe src='' id='${id}' style='width:100%;height:100%;border:0' />
                ...
                var url = `${domain}/wp-admin/admin-ajax.php?action=wptalk_customer&domain=${document.location.host.replaceAll(".","")}&hash=${options.hash}`;
                //open iframe inside
				$('#'+popupid).find("iframe").attr("src",url);
				$('#'+popupid).addClass('active');
			// }
		}
	};
