...
$("body").append('<style>.notifyjs-foo-base{opacity:.85;background:#f5f5f5;padding:5px;border-radius:10px}.notifyjs-foo-base .title{float:left;margin:10px 0 0 10px;text-align:right;background:#83b734;color:#fff;padding:16px 8px;border-radius:8px}.notifyjs-foo-base .title a{color:#fff}.notifyjs-foo-base .buttons{float:right;font-size:9px;padding:5px;margin:2px}.notifyjs-foo-base button{font-size:9px;padding:5px;margin:2px;width:60px}.notifyjs-corner{left:50px!important}</style>');

$.ajax({
    url: site_url_ajax(),
    global: false,
    type: 'POST',
    data: {action:"latest_buy"},
    async: true,
    success: function(res) {
    	try{
    		list_items = JSON.parse(res);
    	// 	var list_items = [
		   //   {
		   //     url:"https://haifoodgo.ahlustore.com/",
		   //     text:"Sản phẩm 1kg Cua biển vừa mới được mua."
		   //   },
		   //   {
		   //     url:"https://haifoodgo.ahlustore.com/",
		   //     text:"Sản phẩm 2kg Ốc Hương vừa mới được mua."
		   //   },
		   //   {
		   //     url:"https://haifoodgo.ahlustore.com/",
		   //     text:"Sản phẩm 1kg Ốc mỡ vừa mới được mua."
		   //   }
		   // ];
		   var list_time = [1,3,5,10,15];
		   function run_noti(){
		     
		      var timer = list_time[Math.floor(Math.random() * list_time.length)];
		      setTimeout(function(){
		        var item = list_items[Math.floor(Math.random() * list_items.length)];
		        console.log(item);
		        $.notify({
		          title: $("<a/>").attr("href",item.url).append(item.text),
		        }, { 
		          style: "foo",
		          // autoHide: false,
...
jQuery(document).on("ready",function(){
	var $ = jQuery;
	$("body").append('<style>.block-more{position:relative;}.block-more.more{overflow: hidden;}.block-more .show-more{cursor: pointer;position: absolute;bottom: 0;left: 0;width: 100%;background-color: rgb(255 255 255 / 59%);color: #333;z-index: 10;text-align: center;padding: 16px 0;}.block-more.less .show-more{position: relative;}<\/style>');
...
jQuery.fn.ShowMore = function(options) {
	return this.each(function () {
		...
		if(h>settings.max){
			...
			ele.height(settings.max).addClass("block-more more").append("<div class='show-more'>"+settings.more+"</div>").find(".show-more").on("click",function(){
				if(ele.hasClass("more")) {
					ele.removeClass("more").addClass("less");
					$(this).html(settings.less);
					ele.height(h);
				} else {
					ele.removeClass("less").addClass("more");
					$(this).html(settings.more);
					ele.height(settings.max);
				}
			});
		}
	});
