jQuery(document).ready(function ($) {
    ...
    window.POS_ADDONS = {
        init: function () {
            ...
            $('#add-fee').on('click', function () {
                if ($('#fee-name').val()) {
                    ...
                    $('#modal-add_custom_fee').find('input, .keypad-fee_val').each(function(i, el){
                        $(el).val("");
        ...
        processPayment: function (cart, payment_method) {
            switch (payment_method) {
                case 'angelleye_woo_paypal_here':
                    ...
                    if (typeof cart.create_post == 'undefined') {
                        cart.create_post = [];
                    }
                    cart.create_post.push(data);
                ...
                case 'stripe':
                    ...
                    jQuery('form.woocommerce-checkout').one('submit', function () {
                        if (jQuery('.stripe-source').length > 0) {
                            cart['stripe_source'] = jQuery('.stripe-source').val();
                            APP.processPayment(cart, true);
        ...
        points: {
            ...
            apply_fee: function () {
                var taxable = (typeof $('#taxable-fee:checked').val() !== 'undefined') ? 'taxable' : 'none';
                var fee_type = 'fixed';
                ...
                var fee = {
                    ...
                    total: floatval($('#modal-add_custom_fee .keypad-keyentry').val()),
        ...
        securesubmit: {
            ...
            responseHandler: function (response) {
                ...
                if (response.error) {
                    ...
                    if (typeof ADDONS.cart.create_post == 'undefined') {
                        ADDONS.cart.create_post = [];
                    }
                    ADDONS.cart.create_post.push(data);
