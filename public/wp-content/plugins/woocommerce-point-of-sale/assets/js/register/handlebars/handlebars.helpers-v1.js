...
Handlebars.registerHelper('displayOrderTitle', function() {
    ...
    if( wc_pos_params.user_can_edit_order == true ){
	    ...
	    title += ' by ';
	    if(customer_id > 0 ){
	        title += '<a href="user-edit.php?user_id='+customer_id+'"  target="_blank">';
	        if(shipping.first_name != '' || shipping.last_name != ''){
	        	title += shipping.first_name + ' ' + shipping.last_name;
	        }else if( typeof shipping.username != 'undefined' && shipping.username != ''){
	        	title += shipping.username;
	        }
	        title += '</a>';
...
Handlebars.registerHelper('displayProductItemImage', function() {
	...
	if(variation_id > 0 && v_data ){
		...
		else if( v_data.image && v_data.image.length > 0 ){
...
Handlebars.registerHelper('displayProductItemMeta', function() {
    ...
    for(var name in variation) {
        ...
        if((name == "undefined") || (typeof name == "undefined")) continue;
