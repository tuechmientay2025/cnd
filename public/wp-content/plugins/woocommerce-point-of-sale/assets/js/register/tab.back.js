...
jQuery(document).ready(function ($) {
    ...
    $(document).on("click","#open_new_tab",function(){
        ...
        addnewtab({id:$(".tab_open_number").text().trim(),title:title,tab_limit:$("#tab-limit").val()});
    });
...
function render_tab_html(data) {
   ...
   //render tables
    ...
    var s = data.map(function(v){
        ...
        tab_region.push(`<a href="#table_region_${v.id}" class="button button-primary wp-button-large">${v.name}</a>`);


        return `<div id="table_region_${v.id}" class="table_region-tab">
        ${v.child.map(function(n){
