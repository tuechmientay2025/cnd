jQuery(document).ready(function ($) {
    window.POS_BOOKING = {
        ...
        data: {},
        xhr_rice: null,
        booking_types: ['booking', 'accommodation-booking'],

        init: function () {
            $.each(this.booking_types, function(i, type){
                wp.hooks.addFilter('wc_pos_add_to_cart_handler_' + type, BOOKING.addToCart, 20, 7);
            });
        ...
        persons_field: function () {
            ...
            if (product.booking.has_persons) {
                if (product.booking.has_person_types) {
                    ...
                    $.each(person_types, function(index, person_type){
                        if($.isEmptyObject(person_type)){
                            return true;
                        }
        ...
        resources_field: function () {
            var product = BOOKING.data.adding_to_cart;
            // Resources field
            if (true == product.booking.has_resources && 'customer' == product.booking.resources_assignment) {
                ...
                for (var i = 0; i < resources.length; i++) {
                    var resource = resources[i],
                        additional_cost = [],
                        currencySymbol = $('<textarea />').html(wc_pos_params.currency_format_symbol).text();

                    additional_cost.push('+' + currencySymbol + accountingPOS(resource['base_cost'], 'formatNumber'));

                    if (resource['block_cost']) {
                        cost = currencySymbol + accountingPOS(resource['block_cost'], 'formatNumber');
                        switch (product.booking.duration_unit) {
                            case 'month' :
                ...
                BOOKING.add_field({
                    'type': 'select',
                    'name': 'resource',
                    'label': product.booking.resource_label ? product.booking.resource_label : booking_i18n[18],
        ...
        create_posted_data: function () {
            ...
            if ($('input[name="wc_bookings_field_start_date_time"]').length && $('input[name="wc_bookings_field_start_date_time"]').val() != '') {
                ...
                var timeEl = $("a[data-value='" + data['_time'] + "']").parent(".block");

                if(timeEl.length){
                    data['time'] = this.tConvert(timeEl.data('block'));
                }else{
            ...
            if (product.booking.has_persons) {
                if (product.booking.has_person_types) {
                    var person_types = product.booking.person_types;
                    $.each(person_types, function(i, person_type){
                        if($.isEmptyObject(person_type)){
                            return true;
                        }
                        var field = $('#wc_bookings_field_persons_' + person_type.ID);
                        var persons = field.length ? field.val() : '';
            ...
            if (true == product.booking.has_resources) {
        ...
        get_booking_data_label: function (key, product) {
            ...
            if (key === 'type') {
                return  !empty(product.resource_label) ? product.resource_label : labels[key];
            }
            return labels[key];
        },
        ...
        tConvert: function (time) {
            ...
            if (hr.length > 1) { // If time format correct
                ss = hr < 12 ? ' AM' : ' PM'; // Set AM/PM
                hr = hr % 12 || 12; // Adjust hours
            }
            return hr + ":" + mn + " " + ss; // return adjusted time or original string
        }
        /*****END*****/
