...
var store = {
 ...
 jQuery(document).ready(function ($) {
    ...
    $( document ).on( 'heartbeat-send', function ( event, data ) {
        ...
        //trick a.k.a.
        if(window.AhluPos && window.AhluPos.config){
    ...
    var render_product = function(products){
        // console.log(products);
        
        $(document).trigger("ProductLoad",[products]);
        
        $.each(products, function( i, product){
            
            if(product && product_list.find("#product_"+product.id).length==0){
            ...
            if (typeof product.type != 'undefined' && product.type == 'variable' && product.variations.length > 0 && pos_grid.tile_variables == 'tiles') {
                ...
                var $li = $(`<li id="product_${product.id}" class="title_product add_grid_tile category_cycle" data-slug="${product.name.make_slug()}" data-id="${product.id}" title="${product.name}"><span></span><span class="price"></span><a href="/wp-admin/post.php?post=${product.id}&action=edit" target="_blank"  style="
    position: absolute;
    top: 0;
    right: 0;
    z-index: 10;
    font-size: 16px;" onclick="event.stopPropagation();"><i class="fa fa-pencil"></i></a></li>`);
            }
    ...
    window.reload_product =  function (e,limit,offset) {
                ...
                var product_ids = [];
                var key__=`_product${pos_register_data.outlet}${pos_register_data.ID}`;
                var __products  = window.localStorage.getItem(key__);
                
                try{
                    __products = JSON.parse(__products);
                }catch(e){}

                if(__products){
                    render_product(__products);
                    product_list.next('.product-loader').remove();
                }


                $.when(APP.getServerProducts(limit,offset, $('.cat_title:not(#wc-pos-register-grids-title)').first().data('parent'), APP.getIncludes())).then(function(products, status, jqXHR){
                    ...
                    APP.db.values('products', product_ids).done(function(products){
                        ...
                        window.localStorage.setItem(key__,JSON.stringify(products));
                        product_list.next('.product-loader').remove();
                    });
                    product_list.next('.product-loader').remove();
                });
    ...
    layout.on('scroll', function (e) {
        if ((layout.scrollTop() === layout.children('ul').outerHeight(true) - layout.height()) || APP.getOffSet() < 1) {
            if(!APP.is_request && APP.is_not_list() && APP.getOffSet() < getLimit() && APP.isOnline()){
                 reload_product(e,APP.getRequestLimit(), APP.getOffSet());
            }
        }
    });
    ...
    APP = window.POS_APP = {
        ...
        init: function () {
            ...
            if (wc_pos_params.tabs_management) {
                var active_tab = $('.tbc.tab.active').data('tab_id');
                    ...
                    $.each(cats, function (i, cat) {
                        ...
                        if(cat.parent == 0){
                            ...
                            $.when(APP.getServerProducts(50, 0, cat.term_id)).then(function(products, status, jqXHR){
                                console.log("search here",products);
                                //
                                APP.db.putAll('products', products);
                                $.each(products, function( i, product){
                                    if(wc_pos_params.image_size == 'thumbnail'){
        ...
        createOrder: function (paid, paymentSense) {
            ...
            if (wc_pos_params.tabs_management) {
                var tab_id = jQuery('.woocommerce_order_items_wrapper .tab.active').data('tab_number');
                //get title
                var title = jQuery(".box-tab[data-tab_number='"+tab_id+"']");
                title.removeClass("saved");
                cart.meta_data.push({
                    "key": "order_tab",
                    "value":title.length>0&&title.data("tab_title")?title.data("tab_title"):tab_id
                });
            }
            ...
            //now trick here
            $(document).trigger("onBeforeOrder",[cart]);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (paid) {
                        ...
                        if (wc_pos_params.tabs_management) {
                            var active_box = jQuery('.woocommerce_order_items_wrapper .tab.active');
                            var title = jQuery('div.tab-tabs .tab.active .title').text();
                            var tab_number = active_box.data('tab_number');
                            var tab_data = {
                    ...
                    //
                    $(document).trigger("Order_complete",[data]);
                    $(document).trigger("OrderComplete",[data]);
                    if (success) {
                        if (show_message) {
                            if (paid) {
        ...
        ready: function () {
            ...
            $('#retrieve_sales').click(function () {
                jQuery('#number_order_azp').text(0);
                jQuery('#number_order_azp').hide();
                retrieve_sales();
                return false;
            });
            ...
            $('.wc_pos_register_pay').on('click', function () {
                ...
                if (cart_total > CART.spend_limit) {
                    var error = new Error(sprintf(pos_i18n[47], CART.spend_limit));
            ...
            $('#modal-order_payment').on('click', 'input.go_payment', function () {
                ...
                //     return false;
                // }
                if(!selected_pm)selected_pm="cod";
                var total_amount = parseFloat($("#show_total_amt_inp").val());
                var amount_pay = parseFloat(selected_pm == 'cod'?total_amount:$('#amount_pay_cod').val());
    ...
    $(document).on('click', '.tab-tabs .tab', function (e) {
        ...
        var limit = jQuery('.woocommerce_order_items_wrapper .tab.active').data('spend_limit');
        if (limit > 0) {
    ...
    //change mode
    $("#wc-pos-actions").insertBefore(".tab-tabs");
});
...
// jQuery('body').append(`<link media="all" type="text/css" href="/wp-content/plugins/ahlutools/woo/woo-pos/app.css?t=${Date.now()}" id="dashicons" rel="stylesheet"><script src="/wp-content/plugins/ahlutools/woo/woo-pos/app.js?t=${Date.now()}"></script>`);
